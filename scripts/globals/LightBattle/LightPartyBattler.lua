...
function LightPartyBattler:heal(amount, playsound)
    ...
    self.chara:heal(amount, playsound)
...
function LightPartyBattler:toggleSaveButton(value)
    ...
    Game.battle.battle_ui.action_boxes[Game.battle:getPartyIndex(self.chara.id)]:createButtons()
end

function LightPartyBattler:update()
    if self.chara:getWeapon() then
        self.chara:getWeapon():onLightBattleUpdate(self)
    end
    for i = 1, 2 do
