...
function LightBattleUI:init()
    ...
    self.status_display = LightStatusDisplay(0, 390, Game.battle.encounter.event and not Game.battle.encounter.show_battle_ui and not Game.battle.multi_mode)
    ...
    elseif state == "ENEMYSELECT" then
        ...
        if self.draw_mercy then
            if self.style == "deltarune" then
                ...
                self:drawCustomGaugeLabels(arena_ox, arena_oy)
            end
        end
        ...
        for index = page_offset + 1, math.min(page_offset + 3, remainder) do
            ...
            if enemy then
                ...
                elseif self.style == "deltarune" then
                    ...
                    self:drawCustomEnemyGauge(enemy, y_offset, hp_percent, arena_ox, arena_oy)
                end
            else
                local enemy_special_text = self.enemies_special_text[index - page_offset]
                enemy_special_text:setText("")
...
-- This is used for drawing custom gauge styles like deltatraveler's (which was moved to a new addon lib)
function LightBattleUI:drawCustomGaugeLabels(arena_ox, arena_oy) end
function LightBattleUI:drawCustomEnemyGauge(enemy, y_offset, hp_percent, arena_ox, arena_oy) end
