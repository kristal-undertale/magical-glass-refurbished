        ...
        if self.state == "PARTYSELECT" then
            if Input.pressed("right") then
                if Mod.libs["moreparty"] then
                    self.party_selecting_spell = Mod.libs["moreparty"]:partySelectMovement(self.party_selecting_spell, "right", true)
                else
            ...
            if Input.pressed("left") then
                if Mod.libs["moreparty"] then
                    self.party_selecting_spell = Mod.libs["moreparty"]:partySelectMovement(self.party_selecting_spell, "left", true)
                else
                    self.party_selecting_spell = self.party_selecting_spell - 1
                end
            end
            ...
            if Mod.libs["moreparty"] then
                if Input.pressed("up") then
                    self.party_selecting_spell = Mod.libs["moreparty"]:partySelectMovement(self.party_selecting_spell, "up", true)
                end

                if Input.pressed("down") then
                    self.party_selecting_spell = Mod.libs["moreparty"]:partySelectMovement(self.party_selecting_spell, "down", true)
                end
            end
...
function LightStatMenu:draw()
    ...
    if self.state == "STATS" then
        ...
        local ATK = party:getBaseStats()["attack"]
        local DEF = party:getBaseStats()["defense"]
        local MAG = party:getBaseStats()["magic"]
        ...
        if self.state == "PARTYSELECT" then
            ...
            -- Preserve the original vertical shift based on the total number of party members
            local global_y_offset = (party_count > per_row) and 18 or 56

            Draw.printAlign("Cast " .. spells[self.spell_selecting]:getName() .. " on", 150, 231 + global_y_offset, "center")

            self.view_start_row = self.view_start_row or 0

            local selected_index = self.party_selecting_spell - 1
            ...
            for i, party in ipairs(Game.party) do
                ...
                if display_row >= 0 and display_row <= 1 then
                    ...
                    -- Dynamic X and Y based on your original formulas
                    local x = 63 - (boxes_in_this_row - 2) * 70 + (col * width)
                    local y = 269 + global_y_offset + (display_row * 38)
                    ...
                    if i == self.party_selecting_spell then
                        Draw.setColor(Game:getSoulColor())
                        -- The heart is always exactly offset from the text's base coordinates
            ...
            -- Indicator
            if Mod.libs["moreparty"] then
                local selected_row = math.floor((self.party_selecting_spell - 1) / Mod.libs["moreparty"]:getPartyPerRowAmount()) + 1
                ...
                if total_rows > 2 then
                    for i = 1, total_rows do
                        ...
                        if selected_row == i then
                            ...
                            love.graphics.rectangle("fill", 337 + (Mod.libs["moreparty"]:getPartyPerRowAmount() == 4 and 60 or 0), 18 + 232 + percentage * 102, 8, 8)
                        else
                            Draw.setColor(COLORS.gray)
                            love.graphics.rectangle("fill", 339 + (Mod.libs["moreparty"]:getPartyPerRowAmount() == 4 and 60 or 0), 18 + 234 + percentage * 102, 4, 4)
                        end
                    end
                end
            end
