...
function PartyMember:init()
    ...
    -- Correctly applies equipment when the party member load in for the first time
    self.mg_initialized = false

    -- Visible in multi-mode battles and light party select menus
    -- Should be at most 6 characters
    self.short_name = nil

    -- Whether this party member will have the player of Undertale
    ...
    -- Support for existing save files
...
function PartyMember:convertToLight()
    ...
    elseif Kristal.getLibConfig("magical-glass", "health_conversion") == nil then -- Kristal default health conversion
...
function PartyMember:setLightLV(level, force_exp)
    ...
    -- Takes into account stat points that were added/removed using PartyMember:increaseStat()
...
-- Updates the party member stats to their current level
...
-- Works with the LV system
-- Use "reset" at the 'amount' parameter to reset the stat buff back to 0
...
-- Triggers when striking an enemy in a light battle
